...
- **Testability pattern**: `AppModeManager` takes `(defaults:, policyManager:, windowOpener:)` — all with production defaults. Never pass these explicitly in app code; only in tests. Same pattern for `PreferencesView`'s `launchAtLogin:` parameter.
- **Test isolation**: Always use `UserDefaults(suiteName: UUID().uuidString)!` in tests — never `.standard`. The suite is discarded after the test, leaving no persistent state.
- **Mock location**: `MockPolicyManager`, `MockWindowOpener`, `MockLaunchAtLogin` are defined inline in their respective test files — no shared `TestHelpers/` needed given the small scope.
